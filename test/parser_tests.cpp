...
{
    gcode_program p = parse_gcode("( This is a comment )\n M23 ( And so is this ) G54");
...
{
    gcode_program p = parse_gcode("( This is a comment G2 )\n M23 ( This is a comment G2 ) G54");
...
{
    gcode_program p = parse_gcode("(*** Toolpath 1 ***)\n G0 X0.0 Y0.0 Z0.0 \n N103 G1 X1.0 F23.0\nG1 Z-1.0 F10.0");
...
{
    gcode_program p = parse_gcode(";Generated with Cura_SteamEngine 2.5.0\nM190 S60\nM104 S200\nM109 S200\nG28 ;Home");
...
{
    gcode_program p = parse_gcode_saving_block_text("(*** Toolpath 2 ***)\n G0 X1.5 Y0.0 Z0.0 \n N103 G1 X1.0 F23.0\nG1 Z-1.0 F10.0");
...
{
    gcode_program p = parse_gcode_saving_block_text("%");
...
{
    gcode_program p = parse_gcode("G99 G82 R0.1 Z-0.1227 P F15.04");
...
{
    gcode_program p = parse_gcode("F10 XY [SET FEEDRATE FOR X AND Y]");
