...
enum chunk_type { CHUNK_TYPE_COMMENT, CHUNK_TYPE_WORD_ADDRESS, CHUNK_TYPE_PERCENT, CHUNK_TYPE_WORD };
...
struct comment_data
{
    ...
    comment_data(const char p_left_delim, const char p_right_delim, const std::string& p_comment_text) : left_delim(p_left_delim), right_delim(p_right_delim), comment_text(p_comment_text) {}
};
...
public:
    ...
    chunk(const char p_left_delim, const char p_right_delim, const std::string& p_comment_text) : chunk_tp(CHUNK_TYPE_COMMENT), cd(p_left_delim, p_right_delim, p_comment_text) {}

    chunk(const char p_wd, const addr p_adr) : chunk_tp(CHUNK_TYPE_WORD_ADDRESS), cd('(', ')', ""), wad(p_wd, p_adr) {}
    ...
    bool equals_comment(const chunk& other_comment) const
    {
        ...
        return (get_comment_text() == other_comment.get_comment_text()) && (get_left_delim() == other_comment.get_left_delim()) && (get_right_delim() == other_comment.get_right_delim());
    }
...
public:
    block(const int p_line_no, const bool p_slashed_out, const std::vector<chunk> p_chunks) : has_line_no(true), line_no(p_line_no), slashed_out(p_slashed_out), chunks(p_chunks) {}

    block(const bool p_slashed_out, const std::vector<chunk> p_chunks) : has_line_no(false), line_no(-1), slashed_out(p_slashed_out), chunks(p_chunks) {}

    block(const block& other) : has_line_no(other.has_line_no), line_no(other.line_no), slashed_out(other.slashed_out)
    {
    ...
    std::vector<chunk>::const_iterator end() const
    {
        return std::end(chunks);
    }
